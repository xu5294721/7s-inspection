...
- Rendering attempt: the packaged `render_docx.py --emit_pdf --verbose` could not start because the environment lacks Python `pdf2image`; no LibreOffice/`soffice` executable is installed. A four-page `representative.pdf`/`pages/page-1.png` through `page-4.png` render already present in the same QA directory was inspected at 100%: page 1 keeps the caption with its single frame; page 2 shows two equal frames and a 2+1 three-photo grid without overlap; page 3 shows a 2x2 four-photo grid and a complete single-photo boundary item; page 4 contains only the closing signature block with no clipping. The canonical renderer limitation remains an environment concern.
- `app/`: `pnpm exec vitest run --maxWorkers=1` passed (`52` files, `635` tests); `pnpm run lint` exited `0`; `pnpm run build` exited `0` (Vite emitted only the existing large-chunk warning).
