...
当前版本：[v1.1.23](https://github.com/xu5294721/7s-inspection/releases/tag/v1.1.23)。支持网页/PWA 和 Android APK 两种使用方式。
...
- 按需选择检查内容：打开检查内容时默认带出“环境卫生-干净整洁”，其他大项按模板设置预选；用户仍可改选或清空。
...
- Word 通报：一键生成 `.docx` 文件；有内容的无照片项同样会写入对应章节。自适应模式下首页优先展示首个有照片项点，后续每页固定展示两个项点，并按照片数量自动压缩排版以减少留白。
...
3. 点击“检查内容”，默认已带出“环境卫生-干净整洁”，可按现场情况改选、清空或补充其他检查大项。
...
从 [v1.1.23 Release](https://github.com/xu5294721/7s-inspection/releases/tag/v1.1.23) 下载 APK，传至 Android 手机后，在“下载”目录中点击安装。如系统提示，请仅对所使用的文件管理器授权“允许安装未知应用”。
