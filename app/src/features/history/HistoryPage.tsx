...
export function HistoryPage() {
  ...
  const drafts = graphs.filter((graph) => graph.inspection.status === "draft");
  const completed = graphs.filter((graph) => graph.inspection.status !== "draft");
  ...
  return <section className="page-section history-page">
    ...
    {drafts.length > 0 ? <section className="history-section" aria-label="待继续巡检">
      <h3>待继续巡检</h3>
    ...
    {completed.length > 0 ? <section className="history-section" aria-label="已完成">
      <h3>已完成</h3>
      <ul className="history-list">{completed.map((graph) => <HistoryRow key={graph.inspection.id} graph={graph} pendingCopyId={pendingCopyId} pendingDeleteId={pendingDeleteId} onCopy={(item) => void copyInspection(item)} onDelete={(id) => void moveToTrash(id)} />)}</ul>
    </section> : null}
    {graphs.length === 0 ? <p className="empty-state">暂无巡检记录。</p> : null}
